    ...
    def connect(self, presynapse, postsynapse, w=1):
        ...
        if length_list != 0:
            for i in range(length_list):
                if self.connection_list[i][0].get_id() == presynapse.get_id() and \
                                self.connection_list[i][0].get_type() == presynapse.get_type():
                    flag1 = True
                    for j in range(1, len(self.connection_list[i])):
                        if self.connection_list[i][j].get_id() == postsynapse.get_id and \
                                        self.connection_list[i][j].get_type() == postsynapse.get_type():
                            flag1 = False
                            break
